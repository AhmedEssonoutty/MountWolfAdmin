<template>
  <div>
    <div class="vendors-table">
      ...
      >
        <template #table>
          <tr v-for="(vendor, i) in suspendedVendors" :key="i">
            <td>
              <input
...
const { suspendedVendors } = storeToRefs(vendorStore());

onMounted(async () => {
  await vendorStore().getSuspendedVendors();
});
...
watch(
  ...
  (newVal) => {
    if (newVal.length) {
      suspendedVendors.value = newVal.forEach((element) => {
